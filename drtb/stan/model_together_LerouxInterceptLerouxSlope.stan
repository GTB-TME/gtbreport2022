...
parameters{
  ...
  cholesky_factor_corr[2*(J-1)] L_OmegaT; /* count correlations */
  vector<lower=0>[2*(J-1)] omegaT;        /* RVs for Sigma */
  real<lower = 0, upper = 1> alphti;   /* time Leroux parms */
}
...
model {
  ...
  phi ~ sparse_leroux(alphsp,E,Dvec,lambda,
                      ...
                      N,2*(J-1)+1,Nedges);
  L_OmegaT ~ lkj_corr_cholesky(2);
  omegaT ~ normal(0, 2.5);
  delta ~ sparse_leroux(alphti,E,Dvec,lambda,
                      diag_pre_multiply(omegaT, L_OmegaT),
                      N,2*(J-1)+1,Nedges);
