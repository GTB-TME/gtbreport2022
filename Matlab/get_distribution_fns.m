...
try
    ...
    % Ensure that the input vector has elements in increasing order
    dat  = sort(prctiles);
    opts = optimset('TolX',1e-12,'TolFun',1e-12,'MaxIter',1e6,'MaxFunEvals',1e6);
    
    % Rough estimators to help setting up initial guesses
    mn = dat(2); var = (dat(3)-dat(1))^2/4;
    
    % Depending on the choice of distribution (log-normal or beta),
    % optimise the distribution parameters so that the appropriate cumulative densities
    % agree as closely as possible with the input percentiles
    ...
    % Plot a comparison between estimated distribution parameters and the
    % input percentiles, to help check that the former reasonably captures 
    % the latter 
